...
async function saveResultToBackend(totalScore, pct, tierLabel) {
  try {
    ...
    if (res.ok) {
      console.log('✅ Result saved:', data.id, 'verified:', data.verified);
      ...
      console.warn('⚠️ Save failed:', data.error);
      const $status = document.getElementById('save-status');
      if ($status) { $status.textContent = '⚠️ Could not save result.'; $status.style.color = 'var(--rose)'; }
    }
  } catch (err) {
